...
def get_locations():
    ...
    LogOutput(f"Received clientId: {client_id}")

    if not client_id:
        LogOutput("No clientId provided, returning 400 error.")
    ...
    # Log the headers to ensure correct setup
    LogOutput(f"Using headers: {headers}")
    ...
    if response.status_code == 200:
        LogOutput("Successfully retrieved locations.")
        return jsonify(response.json()), 200
    else:
        error_details = f"Failed to fetch locations: {response.text}"
        LogOutput(error_details)
